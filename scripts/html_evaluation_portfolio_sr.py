...
def df_yahoo_data(list_securities, start_date = pd.to_datetime('2007-01-03'), end_date = date.today()):
    df = pd.DataFrame()
...
if __name__ == '__main__':
    ...
    data_iv_port_rets.dropna(inplace = True)
    ...
    if debug_xl == True:
        with pd.ExcelWriter('AW_dbg.xlsx',  date_format = 'dd-mm-yyyy', 
            ...
            inv_volat_data.to_excel(writer, sheet_name='inv_vol')        
            data_norm_weights.to_excel(writer, sheet_name='inv_vol_wtd_rets')
            data_iv_port_rets.to_excel(writer, sheet_name='AW_port_rets')
            data_eq_weight_rets.to_excel(writer, sheet_name='EqW_port_rets')
        
        
    qs.reports.html(data_iv_port_rets['Tot'], benchmark = data_eq_weight_rets['Tot'] ,
